...
| [ ] | Bark 推送通知配置 | 设置 → 通知页签 → 「Bark 推送通知」卡片<br>`GET/PUT /admin/notifications/bark`、`POST /admin/notifications/bark/test`（管理员鉴权；`device_key` 永不回显，用 `has_device_key` 表示已配置）<br>`views/admin/settings/BarkNotifySettingsCard.vue`、`api/admin/notifications.ts`、i18n `admin.settings.notifications.bark.*`；后端 Bark 通道与告警引擎接线见批次 6 / A6-2 后端提交 | 服务器地址留官方 `https://api.day.app`，设备 Key 从手机 Bark 应用首页复制 → 点「测试连接」应提示服务器可达并显示延迟（还没填设备 Key 时点它 → 蓝色信息条提示「服务器连通正常，尚未配置设备 Key」，不算报错）；点「发送测试通知」→ 手机应马上收到一条「Sub2API 测试通知」。**两个测试按钮都用表单当前值，不需要先保存**。点「保存」→ 刷新页面：设备 Key 输入框为空但带「已配置」标记，其余字段回显；再保存一次留空设备 Key → 旧 Key 应仍有效（再点发送测试能收到）。只打开「启用」不填设备 Key 直接点保存 → 应被拦下并提示，不发请求。把打断级别切到「紧急」再发一次测试 → 手机静音状态下也应响（这是 critical 的语义，普通告警别用）。最后建一条会触发的告警规则 → 触发和恢复各收到一条推送；关掉「告警恢复时也通知」后只收触发那条 | 中 |
| [ ] | 账号用量提醒规则 | 设置 → 通知页签 → Bark 卡片下方的「账号用量提醒规则」卡片<br>规则走 `GET/POST /admin/ops/alert-rules`、`PUT/DELETE /alert-rules/:id`（新增 metric_type `account_window_used_percent` / `account_quota_used_percent` / `account_balance` / `account_today_cost`，filters 带 `window` / `dimension` / `provider` / `platform` / `group_id` / `account_ids`）；试算 `POST /admin/ops/alert-rules/:id/evaluate`（body `{"send": bool}`）<br>后端 `service/ops_alert_account_metrics.go`、`ops_alert_evaluator_service.go`；前端卡片 `views/admin/settings/AccountUsageAlertRulesCard.vue` | 前提：运维监控开着（`ops_monitoring_enabled`），关着时卡片应提示"需开启运维监控"而不是空表。建一条「5 小时窗口用量 >= 80%，平台 openai」的规则 → 点「立即试算」→ 弹窗按账号列出当前值与是否越阈（没有用量快照的账号不出现、也不会算成 0）；勾「推送到 Bark」再试一次 → 手机收到标题带「手动试发」的通知，首行写明不是真实告警；Bark 没开时应提示未启用而不是静默。等评估周期跑过后，**每个越阈账号各自出一条告警事件**（运维监控 → 告警事件里的描述带账号名与平台），推送正文也带账号行；账号窗口重置后各自收到「已恢复」。Claude 账号的用量按百分比显示（后端已把 0-1 小数换算过来），不会出现 80% 规则永远不触发的情况。再试「一键三档」（40 / 60 / 80）→ 生成三条同名后缀规则，重名会逐条提示。余额规则只对 kimi / deepseek 有效，建议运算符用 `<` | 中 |
