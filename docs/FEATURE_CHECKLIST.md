...
| [ ] | 从 Key 列表一键「查用量」 | `/keys` → 行内「查用量」（紧挨「使用密钥」）<br>`POST /api/v1/key-usage/session`（用原始 Key 换只读令牌）→ 新标签页打开 `/key-usage?t=<令牌>`<br>`KeysView.vue`（openKeyUsage）、`api/keyUsage.ts`（createKeyUsageSession） | 点行内「查用量」→ 按钮先转圈并置灰（此时再点无效），随后**新标签页**打开用量页并直接出该 Key 的结果，不用再粘 Key。**新标签页地址栏必须是 `?t=<令牌>`，绝不能出现原始 Key 串**。额度耗尽/已过期的 Key 照常能开；被**停用**的 Key 后端会拒绝 → 页面弹红色「Invalid or expired key」且不开新标签页（前端不拦截，由后端定）。后端关掉 `key_usage.enabled` 再点 → 同样只弹错误、不开标签页 | 低 |
