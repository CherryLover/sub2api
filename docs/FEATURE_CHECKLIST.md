...
> **裁剪目标（2026-09-08 调整）：内部部署、自用多用户。** 原目标写作「单管理员、内部部署」；站长要给其他人创建多个用户使用，**包括多个管理员**。
> 因此下面这些是**多用户保留面**，不再裁、每次发版都要回归（集中条目见文末「多用户保留面回归」）：
>
> - **用户管理**：`/admin/users` 的创建 / 编辑 / 停用 / 删除 / 重设密码 / 批量并发与 RPM（第 1 章）
> - **角色**：`admin` / `user` 两种；**多个管理员已支持**——新建 / 编辑弹窗的角色下拉都有「管理员」，后端 `POST/PUT /admin/users` 接受 `role=admin`，
>   提权走 step-up 2FA，且不能自我降级、不能降级最后一个管理员、不能禁用管理员（第 1 章「创建第二个管理员」行）
> - **分组授权**：`user_allowed_groups` 授权名单、专属分组、替换分组（第 2 章）
> - **每用户额度**：按平台的日 / 周 / 月平台额度与「用户默认值」（第 1、5 章）
> - **每用户密钥**：用户自己的 `/keys`、管理员侧的「API 密钥」弹窗与跨用户「密钥总表」（第 2 章）
> - **用户侧四个页面**：`/dashboard` `/usage` `/keys` `/profile`（第 1、4 章；管理员访问前两个会转到管理端对应页，见第 4 章）
>
> 仍然裁掉 / 保持关闭的（不会回来）：注册（管理员建号）、支付 / 订阅 / 余额 / 兑换码 / 推广、第三方登录、邮件体系、内容审计、批量生图、公告、模型广场、自定义端点。
...
自定义端点（`custom_endpoints`，批次 6 第三轮整键删除、迁移 240 清库；Keys 页端点弹层只剩当前站点这一个内置端点）
...
| [ ] | 管理员新建用户 | `/admin/users` → 右上「新建用户」<br>`POST /admin/users`<br>`components/admin/user/UserCreateModal.vue`、`handler/admin/user_handler.go:212` | 填邮箱、密码、用户名、并发数、RPM 上限、可用分组，角色选普通用户 → 列表出现。**退出登录用这个新账号真登一次**，能进 `/dashboard`。再建一个选「管理员」→ 二次验证开着时应额外弹验证码框。注册体系删除后，这里是唯一的开户途径 | 中 |
| [ ] | 管理员编辑用户 | `/admin/users` → 行内编辑；可用分组另有独立弹窗<br>`PUT /admin/users/:id`<br>`UserEditModal.vue`、`UserAllowedGroupsModal.vue`、`handler/admin/user_handler.go:247` | 改用户名和备注 → 保存后列表显示新值。重设密码 → 新密码能登、旧密码失败。把角色从普通用户改成**管理员** → 二次验证开着时应弹验证码框（提权保护）；编辑一个本来就是管理员的人时**不应**被打断。用「可用分组」弹窗勾掉一个分组 → 他建 Key 时该分组应不可选 | 中 |
| [ ] | **创建第二个管理员（多管理员）** | `/admin/users` → 「新建用户」角色选「管理员」，或编辑现有用户把角色改成「管理员」<br>`POST /admin/users` / `PUT /admin/users/:id`（`role` 校验 `oneof=admin user`，`user_handler.go:75` / `:88`）<br>`UserCreateModal.vue:30-32`、`UserEditModal.vue:95-97`、`service/admin_user.go:104-111`（normalizeUserRole）、`:146-160`（ensureNotLastAdmin） | **已支持（2026-09-08 读码核实，不需要改代码）。** ①新建一个角色为「管理员」的用户 → 二次验证开着时先弹验证码 → 列表角色列显示「管理员」；**退出后用他登录 → 应进 `/admin/dashboard`**，左侧是完整的管理菜单，能打开用户管理 / 账号管理。②用这个新管理员去编辑原管理员：把原管理员改成普通用户 → 应成功（降级其他管理员是允许的）；再把自己改成普通用户 → 应被拒「cannot demote yourself from admin」。③把系统里改到只剩一个管理员后再试降级他 → 应被拒「cannot demote the last admin user」。④管理员行内**没有**「停用」与「删除」按钮（`UsersView.vue:565` / `:658` 对 `role=admin` 隐藏），用接口硬传 `PUT {"status":"disabled"}` 或 `DELETE` → 后端应拒「cannot disable admin user」/「cannot delete admin user」。⑤审计日志里应有「admin user created」/「user role changed」记录 | 中 |
...
| ✓ | 功能 | 入口 / 接口 / 文件 | 验证方法 | 风险 |
|---|---|---|---|---|
| [ ] | API Key 列表（搜索/筛选/排序/自定义列） | `/keys`；侧边栏「我的账户」→ API 密钥（简易模式下挪到管理员菜单末尾）<br>`GET /api/v1/keys?page&search&status&group_id&sort_by`（`sort_by` 可取 name/id/status/expires_at/last_used_at/created_at/current_concurrency/today_cost）<br>`views/user/KeysView.vue`、`handler/api_key_handler.go:106`、`repository/api_key_repo.go`（apiKeyListOrder） | ①搜索框输 Key 名片段，只剩匹配项；②状态选「已停用」只剩 inactive；③分组下拉选具体分组只剩该组，选「未分组」只剩没分组的；④「列设置」勾掉创建时间，**刷新后仍隐藏**（存在 localStorage 的 `api-key-hidden-columns`）；⑤点表头「创建时间」切换升降序，首行随之变化；⑥点表头「用量」按**今日用量**切换升降序（发 `sort_by=today_cost`，服务端按 usage_logs 当天 actual_cost 汇总排序，没用量的 Key 记 0，翻页仍按该顺序）；⑦「最后使用时间」列默认显示（老用户首次打开会自动放出来一次——列设置版本 3 → 4 时一次性放出，之后可再隐藏），点表头切换升降序，行为同「创建时间」。⑥⑦ 自 v0.1.186 起生效，升级不带迁移 | 无 |
...
| [ ] | Key 用量展示（今日/近 30 天/累计消费） | `/keys` 列表「用量」列<br>`POST /api/v1/usage/dashboard/api-keys-usage`（批量，挂 Heavy 限流）；「累计消费」直接读列表返回的 `quota_used`<br>`KeysView.vue`、`api/usage.ts` | 打开 `/keys`，Network 里应看到**一次**批量请求，请求体是当前页的 key id 数组，**不是每行一个请求**。用某把 Key 跑一次对话再刷新 → 该行「今日」「近30天」「累计消费」三个金额都增加。注意「近30天」来自 usage_logs 的 30 天窗口，「累计消费」是 `api_keys.quota_used` 全量累加，两者本来就可能不等。翻第二页应再发一次批量请求（只带第二页 id） | 无 |
...
| [ ] | 从 Key 列表一键「查用量」 | `/keys` → 行内「查用量」（紧挨「使用密钥」）<br>`POST /api/v1/key-usage/session`（用原始 Key 换只读令牌）→ 新标签页打开 `/key-usage?t=<令牌>`<br>`KeysView.vue`（openKeyUsage）、`api/keyUsage.ts`（createKeyUsageSession） | 点行内「查用量」→ 点击瞬间先弹一个空白新标签页，同时按钮转圈置灰（此时再点无效）；拿到令牌后该标签页自动跳到用量页并直接出该 Key 的结果，不用再粘 Key。**新标签页地址栏必须是 `?t=<令牌>`，绝不能出现原始 Key 串**。浏览器拦截了弹窗时 → 应退化为在**当前标签页**打开用量页，而不是报错。被**停用**的 Key → 按钮直接置灰，悬停提示「已停用的密钥无法查询用量」，不发请求（后端对 inactive 一律 401）；额度耗尽 / 已过期的 Key 照常可点、照常能开。后端关掉 `key_usage.enabled` 再点 → 空白标签页自动关闭、页面弹红色错误 | 低 |
...
| [ ] | 免登录页分享链接与限速环 | `/key-usage?t=<令牌>`<br>`GET /api/v1/key-usage/report?token=…`（令牌路径限流放宽到每 IP 每分钟 60 次）<br>`KeyUsageView.vue`（ringItems / detailRows）、`handler/gateway_handler.go` | 用「设了 5h 限速」的 Key 查询后复制带 `?t=` 的链接，换个浏览器打开 → 不用输 Key 直接出结果，有环形进度圈显示该窗口的已用/上限。再用「没设任何限速」的 Key 查 → **环形进度圈整块消失**（没有上限就没有进度可画）。**额度上限相关的展示已全部删除**：不管哪种 Key，页面上都不该再出现「总额度」环或「剩余额度」行，明细第一行应是「今日费用」 | 低 |
...
| [ ] | 用户「API 密钥」弹窗里启停 / 删除 / 编辑 IP 名单（批次 6 · A6-丙） | `/admin/users` → 行内「API 密钥」弹窗，每把 Key 行内多了启停、IP 名单、删除三个操作<br>`PUT /api/v1/admin/api-keys/:id`（`status` / `ip_whitelist` / `ip_blacklist`）、`DELETE /api/v1/admin/api-keys/:id`<br>`components/admin/user/UserApiKeysModal.vue`、`UserApiKeysModal.spec.ts`（5 例，已进 CI 名单） | 打开某用户的弹窗：①点「停用」→ 状态徽标变 inactive，该用户自己的 `/keys` 刷新后同样显示已停用，拿这把 Key 调 `/v1/usage` → 401；再点「启用」恢复。②编辑 IP 名单填 `999.999.1.1` → 应报无效 IP 且不保存；填合法网段保存 → 该用户 `/keys` 里这把 Key 出现 IP 限制小图标。③删除 → 二次确认 → 弹窗列表消失、用户侧 `/keys` 也消失、原 Key 调网关 401。**弹窗里的分组徽标改绑（含自动补授权）行为不变**，见上一行 | 中 |
| [ ] | 管理端密钥总表（跨用户查看 / 启停 / IP 名单 / 删除） | 侧边栏「密钥总表」→ `/admin/api-keys`（简易模式下隐藏；批次 6 · A6-甲，前后端均已交付）<br>页面 `views/admin/ApiKeysView.vue`（`ApiKeysView.spec.ts` 12 例已进 CI 名单）、`api/admin/apiKeys.ts`；列：名称 / 所属用户 / 密钥掩码 / 分组 / 用量 / 状态 / 最后使用 / 创建 / 操作；操作：查用量（跳 `/admin/usage?api_key_id=&user_id=`）/ 启停 / 编辑分组与 IP 名单 / 删除确认<br>`GET /api/v1/admin/api-keys?page&page_size&sort_by&sort_order&user_id&group_id&status&search`（`group_id=0` = 未分组；`sort_by` 白名单与用户侧相同，含 `today_cost`/`last_used_at`，默认 `created_at desc`）、`PUT /api/v1/admin/api-keys/:id`（新增 `status`、`ip_whitelist`、`ip_blacklist`，兼容原 `group_id`/`reset_rate_limit_usage`）、`DELETE /api/v1/admin/api-keys/:id`<br>`handler/admin/apikey_handler.go`、`service/admin_api_key.go`、`repository/api_key_repo.go`（ListAllForAdmin）、门禁 `TestAdminAPIKeySurfaceRegistered` | ①用管理员令牌调总表：每行 `key` 只是掩码（前 6 + `...` + 后 4），**响应任何位置都不能出现明文 Key**；`user` 带 id/email/username，绑了分组的行 `group` 有值。②`sort_by=today_cost&sort_order=desc` 首行应是今天花最多的 Key，翻到第二页顺序衔接；`user_id=<某人>` 只剩他的；`group_id=0` 只剩未分组；`status=inactive`、`search=<名字片段>` 各验一次。③`PUT` 传 `{"status":"inactive"}` → 拿这把 Key 立刻调 `/v1/usage` 应 401（写完就清认证缓存，不用等过期）；再传 `{"status":"active"}` 恢复；传 `{"status":"expired"}` → 400 `INVALID_API_KEY_STATUS`。④`PUT` 传 `{"group_id":2,"ip_whitelist":["999.999.1.1"]}` → 400 `INVALID_IP_PATTERN`，错误信息里点名 `999.999.1.1`，**分组也不能被改掉**（校验在任何写入之前）；传 `{"ip_whitelist":["10.0.0.0/8"],"ip_blacklist":[]}` → 白名单生效、黑名单清空，换个网段发请求应被拒。⑤`DELETE` 后总表里消失，原 Key 调 `/v1/usage` → 401；同一 id 再删一次 → 404。⑥`/admin/audit-logs` 里：调总表记 `admin.api_keys.read`，PUT 记 `admin.api_keys.update`，DELETE 记 `admin.api_keys.delete`。⑦「查用量」不走 `/key-usage/session`（那需要明文 Key），前端直接跳 `/admin/usage?api_key_id=<id>`。⑧**页面侧**：打开 `/admin/api-keys` 应看到所有用户的 Key 混排（掩码列绝不出现明文）；点表头「用量」「最后使用」切换升降序应发 `sort_by=today_cost` / `last_used_at`；用户筛选输邮箱片段只剩他的、分组选「未分组」只剩没分组的、状态选「已停用」只剩 inactive、搜索框输 Key 名片段只剩匹配项；行内「查用量」跳到 `/admin/usage` 且用户与 Key 筛选已带上；启停 / 编辑分组与 IP 名单 / 删除三个操作的结果与 ③④⑤ 一致，删除前有带 Key 名的确认框 | 中 |
...
| [ ] | 账号容量负载明细（批次 6 · 甲方案，前后端均已交付） | `/admin/accounts` → 「容量」列并发徽标（悬停出简版、点击开右侧负载抽屉）<br>`GET /admin/accounts/:id/recent-requests?minutes=15&limit=50`（仅管理端；`minutes` 1..1440、`limit` ≤200）<br>后端 `handler/admin/account_recent_requests.go`、`service/account_recent_requests.go`、`repository/usage_log_repo_account_recent.go`；前端 `components/account/AccountCapacityCell.vue`（hover 300ms 防抖拉 `minutes=15&limit=1`、30 秒缓存）、`components/admin/account/AccountLoadDrawer.vue`、`components/common/SideDrawer.vue`、`AccountsView.vue`（抽屉打开时暂停自动刷新）；`AccountCapacityCell.spec`（5）、`AccountLoadDrawer.spec`（8）已进 CI 名单 | **页面侧**：鼠标停在「容量」格徽标上约 0.3 秒 → 出现「并发 x/y · 最近 15 分钟 N 次 · 最活跃密钥 …」；30 秒内再悬停不该再发请求。点击徽标 → 右侧抽屉：顶部并发 / 等待数，5 / 15 / 60 分钟切换会重新拉数，「按密钥」「按模型」两组 chips 与请求表随窗口变化，下方「同期错误」段只列该窗口内的错误（不是默认 1 小时），有「数据来自使用记录、有秒级延迟」的提示与刷新按钮；抽屉开着时列表的自动刷新倒计时应停住，关掉后恢复。**接口侧**：①用该账号所在分组的 Key 连发 3 次请求，再调 `GET /admin/accounts/{id}/recent-requests?minutes=15` → `total_requests` 至少 +3；`items` 最新一条在最前（`created_at` 倒序、UTC），每条带 `user.email`、`api_key.name`、`model`、`duration_ms`、`first_token_ms`、token 与费用；`by_api_key` 里这把 Key 的 `count` 与之相符、`name` 是 Key 名；`by_model` 按请求里的模型分组。②把 `limit=1` 再调一次：`items` 只剩 1 条但 `total_requests`、`by_api_key`、`by_model` 数字不变（聚合统计整个窗口，不受条数截断）。③`minutes=0`、`minutes=1441`、`limit=0`、`limit=201`、非整数都应 400；不存在的账号 ID 应 404。④响应体里搜 `credentials`、`sk-` 都不该出现（Key 只有 id / name）。⑤`current_concurrency` / `waiting_count` 与账号列表并发列同源，空闲时为 0；`max_concurrency` 等于账号并发设置。⑥数据来自使用记录，请求结束后才落库，有秒级延迟属正常 | 低 |
| [ ] | 账号周限额按自然周重置（批次 6 · A3） | 新建/编辑账号弹窗 → 「配额限制」→ 周限额行<br>`accounts.extra.quota_weekly_*` 随 `POST/PUT /admin/accounts` 提交；升级迁移 `backend/migrations/239_account_weekly_quota_natural_week.sql`<br>`QuotaDimensionRow.vue`、`QuotaLimitCard.vue`、`service/account.go`（GetQuotaResetTimezone / ComputeQuotaResetAt） | ①升级后打开任一**升级前就设了周限额**的 apikey 账号的编辑弹窗：重置方式应显示「固定时间 / 周一 / 00:00 / 服务器时区」而不是「滚动窗口」；`GET /admin/accounts/{id}` 里 `quota_weekly_reset_mode=fixed`、`quota_reset_timezone` 等于设置页显示的服务器时区、`quota_weekly_reset_at` 是下周一 00:00（按该时区换算成 UTC）；本周已产生的用量应还在（列表周额度进度条不归零），上周的用量应已清零。②新建一个 apikey 账号，周限额一填数字，重置方式行自动变成固定/周一/00:00/服务器时区，保存后 `GET` 回显一致。③用该账号真发一次请求，`quota_weekly_used` 应在原值上累加而不是被清零（说明迁移写的重置点和计费同口径），到下周一 00:00 后再发一次才归零重计。日限额仍默认滚动 24h、Key 的 7d 限速不受影响 | 低 |
...
| ✓ | 功能 | 入口 / 接口 / 文件 | 验证方法 | 风险 |
|---|---|---|---|---|
| [ ] | 用户仪表盘 | `/dashboard`；用户端主菜单第一项（**管理员访问会被转到 `/admin/dashboard`**，见下方 A6-乙 行）<br>`GET /api/v1/usage/dashboard/stats`、`/trend`、`/models`、`GET /api/v1/usage`（近期记录）、`GET /api/v1/user/platform-quotas`<br>`views/user/DashboardView.vue`、`components/user/dashboard/*` | 用一个有历史用量的账号打开 → 四块内容都要出：顶部统计卡、趋势图、模型分布、最近 5 条使用记录，右侧快捷操作。改一次日期区间（默认近 7 天）→ 图表应重新拉数（Network 里出现新的 trend/models 请求）。切换粒度（天/小时）图表跟着变。设过平台额度的用户，统计卡里应能看到平台额度余量 | 低 |
| [ ] | 管理员访问用户端仪表盘 / 用量页自动转到管理端（批次 6 · A6-乙） | 管理员登录后直接敲 `/dashboard` 或 `/usage`；「我的账户」侧栏<br>`router/adminRedirect.ts`（`resolveAdminEquivalentPath`，只收录 `/dashboard` → `/admin/dashboard`、`/usage` → `/admin/usage`）、`components/layout/AppSidebar.vue` | 用管理员账号：地址栏敲 `/dashboard` → 落到 `/admin/dashboard`；敲 `/usage` → 落到 `/admin/usage`；左侧「我的账户」分组里**没有**「用量」项，但仍有「API 密钥」与「个人资料」；敲 `/keys` → 照常打开管理员自己的密钥页（**不转**）。再用普通用户账号：`/dashboard`、`/usage` 都正常打开、不被转走。后台模式开着时普通用户仍按后台模式的规则被拦（重定向放在 admin 校验之后、backend_mode 之前） | 中 |
| [ ] | 用户使用记录（含错误请求） | `/usage`；主菜单「用量」（简易模式下隐藏；**管理员访问会被转到 `/admin/usage`**，见上一行）<br>`GET /api/v1/usage`、`/stats`、`/errors`、`/errors/:id`、`/dashboard/snapshot-v2`、`/dashboard/models`（整组挂 Heavy 限流）<br>`views/user/UsageView.vue` | 「使用记录」页签：按日期、模型、API Key 筛选后列表和统计都要跟着变；点某行看详情（token 明细和计费）。切到「错误请求」页签 → 应能看到自己的失败请求（**这一项受系统设置 `allow_user_view_error_requests` 控制，关掉后该页签应无数据或不可见**）。点「导出 CSV」应下载当前筛选结果 | 低 |
...
| [ ] | 使用记录账号可点 / 按账号筛选（批次 6，前后端均已交付） | `/admin/usage` → 「使用记录」账号列是按钮；账号页行内操作菜单「查看用量」<br>`GET /admin/usage?account_id=`（每条记录的 `account` 带 `id / name / platform`）<br>`components/admin/usage/UsageTable.vue`（emit `accountClick`）、`views/admin/UsageView.vue`（`applyRouteQueryFilters` 读 `account_id`，mount 后 `adminAPI.accounts.getById` 回填账号名、兜底 `#id`）、`UsageFilters.vue`（expose `setAccountKeyword`）、`components/admin/account/AccountActionMenu.vue`（「查看用量」）、`handler/dto/types.go`（AccountSummary）；`UsageTable.spec`（+2）、`UsageView.spec`（+4）已进 CI 名单 | ①在使用记录里点某条记录的账号名 → 列表只剩该账号的记录、筛选栏账号框回填该账号名，Network 里 `GET /admin/usage?account_id=…` 返回 200 且每条 `account.id` 都是它；②从账号页「查看用量」进入 `/admin/usage?account_id=…` 应直接带筛选打开且账号名已回填；③`GET /admin/usage` 每条 `account` 里有 `platform` 字段，且整段响应里没有 `credentials`；用户侧 `GET /api/v1/usage` 依旧不带 `account` | 无 |
...
| [ ] | Bark 推送通知配置 | 设置 → 通知页签 → 「Bark 推送通知」卡片<br>`GET/PUT /admin/notifications/bark`、`POST /admin/notifications/bark/test`（管理员鉴权；`device_key` 永不回显，用 `has_device_key` 表示已配置）<br>`views/admin/settings/BarkNotifySettingsCard.vue`、`api/admin/notifications.ts`、i18n `admin.settings.notifications.bark.*`；后端 Bark 通道与告警引擎接线见批次 6 / A6-2 后端提交 | 服务器地址留官方 `https://api.day.app`（站长自建的 `https://notify.flyooo.uk` 是标准 bark-server，直接填它即可，不需要额外鉴权头），设备 Key 从手机 Bark 应用首页复制 → 点「测试连接」应提示服务器可达并显示延迟。这个按钮把探活 `ping_ok` 和推送 `ok` 合起来看，共**四种色调**：两者都成功 → 绿色；还没填设备 Key 时点它 → 后端只探活、不推送，蓝色信息条提示「服务器连通正常，尚未配置设备 Key」，不算报错；探活接口没响应但推送成功（比如自建服务器没开 `/ping`）→ 琥珀色警告「服务器未响应探活接口，但推送可用」；都失败 → 红色「连接失败」。「发送测试通知」只看 `ok`：点它 → 手机应马上收到一条「Sub2API 测试通知」，没填设备 Key 时会在前端直接拦下提示，不发请求。**两个测试按钮都用表单当前值，不需要先保存**。点「保存」→ 刷新页面：设备 Key 输入框为空但带「已配置」标记，其余字段回显；再保存一次留空设备 Key → 旧 Key 应仍有效（再点发送测试能收到）。只打开「启用」不填设备 Key 直接点保存 → 应被拦下并提示，不发请求。把打断级别切到「紧急」再发一次测试 → 手机静音状态下也应响（这是 critical 的语义，普通告警别用）。最后建一条会触发的告警规则 → 触发和恢复各收到一条推送；关掉「告警恢复时也通知」后只收触发那条 | 中 |
| [ ] | 账号用量提醒规则 | 设置 → 通知页签 → Bark 卡片下方的「账号用量提醒规则」卡片<br>规则走 `GET/POST /admin/ops/alert-rules`、`PUT/DELETE /alert-rules/:id`（新增 metric_type `account_window_used_percent` / `account_quota_used_percent` / `account_balance` / `account_today_cost`，filters 带 `window` / `dimension` / `provider` / `platform` / `group_id` / `account_ids`）；试发（干跑）`POST /admin/ops/alert-rules/:id/evaluate`（body `{"send": bool}`；不落事件、不动状态）<br>后端 `service/ops_alert_account_metrics.go`、`ops_alert_evaluator_service.go`；前端卡片 `views/admin/settings/AccountUsageAlertRulesCard.vue` | 前提：运维监控开着（`ops_monitoring_enabled`），关着时卡片应提示"需开启运维监控"而不是空表。建一条「5 小时窗口用量 >= 80%，平台 openai」的规则 → 点「立即试发」→ **先只干跑**：弹窗按账号列出当前值与是否越阈，不产生告警事件、不推送（没有用量快照的账号不出现、也不会算成 0）；勾上「同时推送到 Bark」点「再试一次」→ 手机收到标题带「手动试发」的通知，首行写明不是真实告警；Bark 没开时应提示未启用而不是静默（后端 400 `BARK_NOT_ENABLED`）。等评估周期跑过后，**每个越阈账号各自出一条告警事件**（运维监控 → 告警事件里的描述带账号名与平台），推送正文也带账号行；账号窗口重置后各自收到「已恢复」。Claude 账号的用量按百分比显示（后端已把 0-1 小数换算过来），不会出现 80% 规则永远不触发的情况。再试「一键三档」（40 / 60 / 80，只对百分比指标可用）→ 填基础名称后生成三条名称后缀 40% / 60% / 80%、运算符 >= 的规则；**三条全部创建成功时弹窗自动关闭**，列表里直接出现三条；有失败或重名（本地查重 + 后端重名识别）时弹窗不关，保留逐条结果供核对。余额规则只对 kimi / deepseek 有效，建议运算符用 `<` | 中 |
...
| [x] | ~~一批前端封装零调用方~~ **前端已清理** | — | **已删**（批次 6 WP-C1，每个删前 grep 全前端含 spec 确认零调用）：`api/keys.ts getById`；`api/usage.ts getMyApiKeyDailyUsage / getStatsByDateRange`（连带只为它服务的 `ApiKeyDailyUsagePoint / ApiKeyDailyUsageResponse`）；`api/admin/groups.ts getGroupApiKeys / getStats / getByPlatform / toggleStatus`；`api/admin/users.ts updateConcurrency / getUserUsageStats`；`api/admin/dashboard.ts getRealtimeMetrics / getUsageTrend / getGroupStats / getApiKeyUsageTrend / getBatchApiKeysUsage`（连带 `TrendResponse / GroupStats* / ApiKeyTrend* / BatchApiKey*` 死类型）；`api/admin/userAttributes.ts reorderDefinitions`；`api/admin/ops.ts getErrorLogDetail / updateErrorResolved / updateRequestErrorResolved / updateUpstreamErrorResolved`；`api/totp.ts getVerificationMethod` 与类型 `TotpVerificationMethod`。后端接口 `GET /user/totp/verification-method` **已由 WP-C2（`55734db0e`）删除**（handler + service + 门禁缺席断言），way-rc 实测 404、对照 `/user/totp/status` 401 |

### P2 · 文案与死代码（不影响功能，看着不对）

| ✓ | 残留 | 位置 | 说明 |
|---|---|---|---|
| [x] | ~~`subscription_type` 字段前端还在 8 个文件里传来传去~~ **前端已清理** | — | **已删**（批次 6 WP-C1）：先确认后端非测试代码零出现该字段，再删 `Group.subscription_type`、`SubscriptionType` 类型、`UserAvailableGroup.subscription_type`，以及 KeysView / UserApiKeysModal / GroupSelector / AccountGroupsCell / AvailableChannelsTable 里的 `:subscription-type` 传参；`GroupBadge` / `GroupOptionItem` 删掉永远走不到的订阅配色、剩余天数、"订阅"文字分支及 `subscriptionType / daysRemaining / alwaysShowRate` 三个 prop，顺带删只为它们服务的 `groups.subscription`、`admin.users.daysRemaining / expired` 文案。**数据库 `ent/schema/group.go` 的列仍在，属后端范围** |
...
| [x] | ~~一组失效 i18n 文案零引用~~ **已清理** | — | **已删**（批次 6 WP-C1，中英同步）：`admin.users` 下 `setAllowedGroups / allowedGroupsHint / noStandardGroups / allowAllGroups / allowAllGroupsHint / allowedGroupsUpdated / failedToLoadGroups / failedToUpdateAllowedGroups / insufficientBalance`。同批顺带删掉的零引用文案：`admin.users` 的充值记录一组（`balanceHistory / balanceHistoryTitle / noBalanceHistory / typeBalance …`）、`admin.groups.claudeMaxSimulation` 整块；`admin.usage.clickToViewBalance`（写着「点击查看充值记录」，实际是下钻筛选）改为新键 `clickToFilterByUser` |
| [x] | ~~`views/auth/` 下三份文档还在教人用已删除的注册页~~ **已清理** | — | **已删**（批次 6 WP-C1）：`frontend/src/views/auth/README.md`、`VISUAL_GUIDE.md`、`USAGE_EXAMPLES.md` 整个删掉，全仓库无引用；`views/auth/` 只剩 `LoginView.vue` 与 `index.ts` |
...
| [x] | ~~订阅相关死类型定义~~ **已清理** | — | **已删**（批次 6 WP-C1）：`types/index.ts` 的 `Subscription`、`CreateSubscriptionRequest`、`UpdateSubscriptionRequest`。旁边同样零引用的 `ProxyNode / ConversionRequest / ConversionResult / SubscriptionStats / UserStats`（模板遗留）本轮未动，留给下一轮 |
| [x] | ~~Key 用量脚本里的余额兜底字段~~ **已清理** | — | **已删**（批次 6 WP-C1）：`views/user/KeysView.vue` 生成的用量脚本去掉 `?? response?.balance`，仍按 `remaining → quota.remaining` 取值，行为不变 |
| [x] | ~~审计日志路由白收一个 step-up 参数~~ **已清理** | — | **已删**（批次 6 第二轮 WP-C2，`55734db0e`）：`registerAuditLogRoutes` 去掉被 `_` 忽略的 step-up 参数。同一提交顺带删掉 `config` 层纯死的 `server.frontend_url`（字段、viper 默认值、URL 校验、`warnIfInsecureURL` 死 helper、config_test、`deploy/config.example.yaml`）与 `GET /user/totp/verification-method` 死端点（见上方 P1 表末行） |
| [ ] | 前端零引用的死类型与文案（WP-C1 顺手发现，未动） | `types/index.ts` 的 `ProxyNode / ConversionRequest / ConversionResult / SubscriptionStats / UserStats`；i18n `admin.users` 下 `depositSuccess / withdrawSuccess / failedToDeposit / failedToWithdraw / useDepositWithdrawButtons / amountRequired` | 模板遗留与充值 / 扣款残留，全前端零引用（含 spec）。删前再 grep 一次即可 |
| [ ] | `KeysView.vue` 里只写不读的公开设置 | `views/user/KeysView.vue:1225`（`publicSettings`）、`:1436`（`loadPublicSettings`）、`:1879`（mount 时调用） | 自定义端点删除后 `EndpointPopover` 不再需要公开设置，但页面仍在 mount 时拉一次 `getPublicSettings()` 存进 `publicSettings`，全文件没有任何读取方。多一次无意义请求 |
| [ ] | ops 告警规则的 `notify_email` 与告警事件的 `email_sent` 死字段（后端） | `service/ops_alert_models.go:32`（`NotifyEmail`）、`:58` / `:117`（`EmailSent`）及对应 DB 列 | 邮件体系删除后前端不再提供勾选，新建默认 `false`、永远不会再写成 `true`；存量为 `true` 的行不会被改写，但已无任何消费方。要清得连列一起删（需迁移） |
...
> 同类的刻意保留项（不是漏删，细节见 TRIM_PLAN 第四节「后续候选项台账」）：`usage_logs.subscription_id` / `groups.subscription_type` / `users.balance` 家族三处列，
> `registration_enabled` / `channel_monitor_mode` 两个回滚兜底键，`LOGIN_ENTRY_RESERVED_PREFIXES` 里的 `/legal`、`/custom`。
...
### 多用户保留面回归（2026-09-08 目标调整后新增）

裁剪目标改为「内部部署、自用多用户」之后，这些不再是"顺便还活着"的功能，而是**必须一直好用**的保留面。每次发版至少过一遍。

| ✓ | 条目 | 所在章节 | 为什么 |
|---|---|---|---|
| [ ] | 创建普通用户 + 创建第二个管理员 | 1 | 注册已删，管理员建号是唯一开户途径；多管理员是新目标的核心诉求。新建的普通用户要真登进 `/dashboard`，新建的管理员要真登进 `/admin/dashboard` |
| [ ] | 管理员的防锁死保护 | 1 | 不能自我降级、不能降级最后一个管理员、不能禁用管理员；提权走 step-up。多个管理员互相操作时这几条是唯一护栏 |
| [ ] | 普通用户与管理员分别登录 | 1 / 4 | 普通用户落 `/dashboard`；管理员落 `/admin/dashboard`，且敲 `/dashboard` `/usage` 会被转到管理端、`/keys` 不转（A6-乙） |
| [ ] | 分组授权（专属分组撤销后当场 403） | 2 | 多用户下靠它隔离谁能用哪个分组；只认 `user_allowed_groups` 授权行 |
| [ ] | 每用户平台额度 | 1 | 多用户下唯一的按人限额手段（Key 额度是按 Key 的）；用尽拒绝、重置窗口恢复 |
| [ ] | 每用户密钥：用户自己的 `/keys` 与管理员侧「API 密钥」弹窗 | 2 | 用户建 / 改 / 停 / 删自己的 Key；管理员在弹窗里能启停 / 删除 / 改 IP 名单（A6-丙），两边状态要一致 |
| [ ] | 密钥总表跨用户可见性 | 2 | `/admin/api-keys` 要能看到**所有**用户的 Key、按用户筛选、掩码绝不泄露明文；普通用户调 `GET /api/v1/admin/api-keys` 必须被拒 |
