...
| [ ] | 账号容量负载明细（批次 6 · 甲方案） | `/admin/accounts` → 「容量」列并发徽标 → 右侧负载抽屉（前端另行交付）<br>`GET /admin/accounts/:id/recent-requests?minutes=15&limit=50`（仅管理端）<br>`handler/admin/account_recent_requests.go`、`service/account_recent_requests.go`、`repository/usage_log_repo_account_recent.go` | ①用该账号所在分组的 Key 连发 3 次请求，再调 `GET /admin/accounts/{id}/recent-requests?minutes=15` → `total_requests` 至少 +3；`items` 最新一条在最前（`created_at` 倒序、UTC），每条带 `user.email`、`api_key.name`、`model`、`duration_ms`、`first_token_ms`、token 与费用；`by_api_key` 里这把 Key 的 `count` 与之相符、`name` 是 Key 名；`by_model` 按请求里的模型分组。②把 `limit=1` 再调一次：`items` 只剩 1 条但 `total_requests`、`by_api_key`、`by_model` 数字不变（聚合统计整个窗口，不受条数截断）。③`minutes=0`、`minutes=1441`、`limit=0`、`limit=201`、非整数都应 400；不存在的账号 ID 应 404。④响应体里搜 `credentials`、`sk-` 都不该出现（Key 只有 id / name）。⑤`current_concurrency` / `waiting_count` 与账号列表并发列同源，空闲时为 0；`max_concurrency` 等于账号并发设置。⑥数据来自使用记录，请求结束后才落库，有秒级延迟属正常 | 低 |
...
| [ ] | 使用记录账号可点 / 按账号筛选（批次 6） | `/admin/usage` → 「使用记录」账号列可点；账号页操作菜单「查看用量」（前端另行交付）<br>`GET /admin/usage?account_id=`（每条记录的 `account` 带 `id / name / platform`）<br>`components/admin/usage/UsageTable.vue`、`UsageFilters.vue`、`handler/dto/types.go`（AccountSummary） | ①在使用记录里点某条记录的账号名 → 列表只剩该账号的记录、筛选栏账号框回填该账号名，Network 里 `GET /admin/usage?account_id=…` 返回 200 且每条 `account.id` 都是它；②从账号页「查看用量」进入 `/admin/usage?account_id=…` 应直接带筛选打开且账号名已回填；③`GET /admin/usage` 每条 `account` 里有 `platform` 字段，且整段响应里没有 `credentials`；用户侧 `GET /api/v1/usage` 依旧不带 `account` | 无 |
