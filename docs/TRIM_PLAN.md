...
| 3. Key 认证与额度引擎 | 🟢 大部分完成 | 防爆破/IP ACL/幂等结算/惰性窗口/时区均已存在；额度已在批次 4 统一到 `api_keys.quota`；**待做=周额度改自然周锚点（A3，批次 6，截至 2026-09-07 未开始）** |
...
**下一步**：A3 周额度自然周锚点（未开始）；A6 剩余部分（Keys/用量页单管理员语义中除 A6-1 / A6-2 / A6-3 之外的部分，**范围待定**）；
...
| **批次 6** | A3 周额度自然周锚点、A6 后台单管理员化、A7 文档改写 + 压测 / 备份恢复 / Key 泄露演练 | 🔵 **进行中：A6-1（v0.1.184）、A6-2 第一步（v0.1.185）、A6-2 第二步 + 密钥页排序（v0.1.186）已上线**（2026-09-07 生产与 way-rc 均在 0.1.186，发版与升级记录见第六节「批次 6 发版记录」）；**A3 / A7 / A6 剩余未开始** |
