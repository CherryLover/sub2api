...
### ⚠️ 分支与 main 已严重分叉：验证的镜像 ≠ 合并后的产物（2026-08-28 发现）

裁剪分支与 `origin/main` 的共同祖先是 `35482567`（2026-08-21），此后：

- **main 前进了 257 个提交**，其中包括 **Go 1.27.0 升级**（`cbe258fd1`，同步改了 CI、Dockerfile、
  golangci-lint v2.13 与 jsonv2 适配）。裁剪分支仍停在 **Go 1.26.6**。
- **迁移号已经撞车**：分支有 `229_force_registration_disabled.sql`，
  main 有 `229_plugins.sql` + `230_plugin_artifacts.sql` + `231_add_usage_log_requested_reasoning_effort.sql`
  + `231_user_restrict_public_groups.sql`。合并后同一编号会存在多个文件。

**后果**：本流程"出 RC 镜像 → way-rc 验证通过 → 合并 main"的前提是"验的就是要发的"。
分叉 257 个提交后这个前提不再成立 —— **验证通过的镜像与合并 main 后自动发版产出的 `:latest` 是两份不同的代码**，
而 `:latest` 是生产的活动标签（见第一节 ⚠️）。

**建议**（批次 3 开工前处理）：先把 main 合并/rebase 进裁剪分支并让 CI 全绿，再开始批次 3；
否则分叉只会随批次 3/4/5 继续扩大，最终合并的风险远高于现在处理。
...
- [ ] 第 2 节 空库全新部署（需另起一套容器名不冲突的临时栈）
- [ ] 第 3 节 初始化验证（AUTO_SETUP、`/setup` 跳转、TOTP 可选项）
- [ ] 第 4.2 / 4.3 / 4.4 节 已裁剪路由 404、公开设置不泄露已裁剪键、登录页无第三方按钮
- [ ] 新增：B1 / B2 / B4 的裁剪面探测（合规确认、站点品牌设置项、人机验证）
- [ ] 新增：存量用户仍可登录的回归（way-rc 库现有 2 个用户：admin + test1）
- [ ] 第 6 节 老库升级演练
- [ ] **第 5 节 核心链路验证——当前被环境阻塞**：way-rc 上唯一的上游账号是
  `rc-fake-claude`（platform=anthropic，status=**error**），是个假账号，无法真发转发请求。
  需站长决定：给 way-rc 配一个真实可用的上游账号，或接受"只验证到上游边界"的降级结论。
