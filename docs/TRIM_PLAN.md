...
生产 `way.flyooo.uk` 已于 2026-09-10 10:30Z 升到 **`0.1.189`**（账号列表调度诊断 + 上游错误列，见第六节最上方「v0.1.189 发版与上线记录」）；way-rc 仍为 `0.1.188`（本次按站长指示跳过 RC）。`fork/main` 现为 `a19cacf4d`（VERSION 已同步 0.1.189）。
...
- [x] **账号列表看不到「账号为什么没被调度」和上游错误**（2026-09-10 公司 Openai-proxy 实例 429 事故中发现；**已处理**，合并提交 `1eb3d30aa`，v0.1.189 已上线生产）：
      状态列在进程内停用 / 额度自动暂停 / 代理隔离时显示「不可调度」，只有单模型冷却时显示「部分受限」，悬停列出原因与恢复时间；
      新增「上游错误」列（默认显示），最近 15 分钟按状态码计数、悬停看最后一次错误的时间 / 状态码 / 模型 / 原文。
      接口 `POST /api/v1/admin/accounts/diagnostics/batch`，诊断只读（新增不改动状态的 peek 方法），无迁移
- [ ] **codex 快照自愈撤销限流时不解除进程内停用**（2026-09-10 事故根因，**未处理**）：
      `handle429` 在 x-codex 头显示 7d 用满时既写库限流、又经 `BlockAccountScheduling` 写进程内停用，截止到周重置
      （实测由 `gpt-5.3-codex-spark` 请求落在母账号上触发，spark 的「额度用完」被当成整号周额度用完）；
      随后 `AccountUsageService.clearOpenAIRateLimitIfCodexSnapshotHealthy` 只清库和调度快照，**不调 `ClearAccountSchedulingBlock`**，
      账号在后台显示正常却一直不被调度，直到重启或管理员点「清除限流」（「测试」按钮只在库里仍有限流时才会解除，不可靠）。
      修法：自愈成功后同步解除进程内停用；spark 模型的 429 不应套用母账号的 7d 窗口
...
### v0.1.189 发版与上线记录（2026-09-10）**通过**

背景：同日公司 Openai-proxy 实例（跑的是裁剪前的旧镜像 `main-6b40e25`，与本仓库生产不是一条线）大量 429，
排查发现账号被网关进程内停用、后台却显示正常，上游错误也无处可看。本版让账号列表直接看得见；根因修复另记在第四节台账。
以下数字全部为 CI 记录与实测，没列出的验证就是没做。

#### 合并与发版

- [x] 分支 `feat/account-list-diagnostics`（`9b7683e34` + `d45b5c731`）：首轮 CI 只挂 golangci-lint（errcheck：测试里 20 处未检查的类型断言，
      CI 只报了 3 处，印证第四节「CI 的 lint 输出不是完整清单」），全部改掉后 CI run 34462585438 四个 job 全绿
- [x] 本地 `--no-ff` 合并到 `fork/main`：合并提交 `1eb3d30aa`（对 `209212baa` 无冲突）；main CI run 34464606965 ✓，
      Auto Release 34465291409 ✓ 算出 **v0.1.189**，Release 34465316405 ✓（amd64 / arm64 镜像 + VERSION 同步提交 `a19cacf4d`）
- [x] 前端 vue-tsc / vitest / eslint 本地通过；本机用模拟后端在浏览器里逐一核对了「不可调度」「部分受限」、悬停原因、上游错误列与「-」兜底

#### 生产升级（2026-09-10 10:30Z，`0.1.188` → `0.1.189`，按站长指示跳过 way-rc）

- [x] 升级前基线：迁移头 240、记录 280、表 77、7 账号 / 10 密钥 / 3 用户、库 134 MB、近 1 小时 0 error；负载 0.46
- [x] 备份 `backups/sub2api-pre-0.1.189-20260910-103024.dump`（5.8 MB；本版无迁移，未做导回演练）；
      compose 备份 `docker-compose.yml.bak-pre-0.1.189-20260910-103024`，只改第 27 行镜像标签
- [x] 镜像标签 `org.opencontainers.image.revision` = `1eb3d30aa…`，与 CI 验证过的合并提交一致；只重建主体容器，**10 秒 healthy**，postgres / redis 全程不停
- [x] 升级后：迁移头 / 记录 / 表数不变（240 / 280 / 77），业务数据不变（7 / 10 / 3），日志零 error / panic，负载 0.5
- [x] 路由：`POST /api/v1/admin/accounts/diagnostics/batch` 未登录 401（与 today-stats 一致），不存在路径 404，`/health` 200
- [x] 站长浏览器登录态实测：侧栏显示 v0.1.189，账号页新增「上游错误」列、控制台零报错；页面自身的诊断请求 200（11 ms）；
      最近 15 分钟无错误故全部显示「-」；把窗口放到 24 小时，#2 返回 1 条（上游 400，`gpt-5.4-mini` 不支持 Codex），与 `ops_error_logs` 一致

**遗留**：way-rc 仍在 0.1.188；codex 快照自愈不解除进程内停用的根因未修（见第四节台账）；
公司 Openai-proxy 实例仍是裁剪前镜像，本版不会自动到达那里，要用需另行决定（整体升级或单独移植）。
...
- [ ] `nanoid` 那条扫描例外形同虚设（本仓库已是修复版 3.3.18，`pnpm audit --prod` 根本不会报它）。
      **更正**：曾一度以为它 2026-10-13 到期会让 Security Scan 失败，读 `tools/check_pnpm_audit_exceptions.py` 后确认不会——
      过期判断（第 216 行）在「遍历 audit 实际报出的漏洞」这个循环里面，例外没被用上就不会被检查过期。
      佐证：`lodash-es`（2026-07-02）与 `axios`（2026-07-10）两条早就过期，Security Scan 至今全绿。
      所以这条是可删可不删的死配置，不是待修项
