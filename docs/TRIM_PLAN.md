...
| **批次 6** | A3 周额度自然周锚点、A6 后台单管理员化、A7 文档改写 + 压测 / 备份恢复 / Key 泄露演练 | 🔵 **进行中：A6-1 已启动、A6-2 第一步已启动（2026-09-04）、A6-2 第二步已启动（2026-09-05）**，其余（A3 / A6 其余部分 / A7）未开始 |

**A6-2 范围说明（Bark 推送 + 告警出口，2026-09-04 启动）**：邮件体系在批次 3 整删后系统没有任何主动推送通道，
运维告警引擎（`OpsAlertEvaluatorService`）触发 / 解除时只落 `ops_alert_events`、不外发。A6-2 分两步补回：

- **第一步（🔵 进行中）：Bark 通道 + 告警出口。** 后端新增 Bark 推送客户端（`bark_notifier.go`，直连、不走上游代理池、10 秒超时）
  与设置服务（`notify_bark_settings.go`，配置整段 JSON 存 `settings.notify_bark_config`，`device_key` 用 `SecretEncryptor` 加密，
  读取永远脱敏、以 `has_device_key` 表示已配置）；管理端三条接口 `GET/PUT /api/v1/admin/notifications/bark`、
  `POST /api/v1/admin/notifications/bark/test`（先 `/ping` 探活、再 push，推送失败回 502）；告警引擎在事件落库后推「触发」，
  解除时按 `notify_on_resolve` 推「已恢复」，推送失败只记 Warn、不影响评估。裁剪门禁加了正向断言（三条路由在场）与
  反向断言（`notify_bark_config` 不进公开设置、旧的 `email-notification` 路由不回流）。前端配置页由另一位实施者并行做。
- **第二步（🔵 进行中，2026-09-05 启动）：账号用量阈值规则。** 把账号用量 / 额度接近上限做成可配置的告警规则，复用第一步的出口。
  范围：告警引擎新增四个按账号拆分评估的指标 `account_window_used_percent`（5h / 7d 滚动窗口，openai / anthropic / grok / kimi / zhipu 快照）、
  `account_quota_used_percent`（日 / 周 / 总额度）、`account_balance`（kimi / deepseek 余额）、`account_today_cost`（今日费用）——
  一条规则 × N 个账号各自触发 / 恢复，事件靠 `dimensions.account_id` 区分，无快照 / 无 limit / 无余额键的账号视为"无数据"跳过；
  新增 `POST /api/v1/admin/ops/alert-rules/:id/evaluate`「立即试算」（不落事件、可选推「手动试发」到 Bark）；
  Bark 正文加账号行。不新增迁移与设置键。前端配置卡片由另一位实施者并行做。
