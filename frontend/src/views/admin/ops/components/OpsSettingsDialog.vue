...
import type { OpsAlertRuntimeSettings, OpsAdvancedSettings, OpsMetricThresholds } from '../types'
...
async function loadAllSettings() {
  loading.value = true
  try {
    const [runtime, advanced, thresholds] = await Promise.all([
...
<template>
  <BaseDialog :show="show" :title="t('admin.ops.settings.title')" width="extra-wide" @close="emit('close')">
    ...
    <div v-else-if="runtimeSettings && advancedSettings" class="space-y-6">
      <!-- 验证错误 -->
