export default {
    settings: {
      ...
      notifications: {
        bark: {
          title: 'Bark Push Notifications',
          description: 'Push alerts to the Bark app on iOS when they fire. Self-hosted Bark servers are supported.',
          enabled: 'Enable Bark push',
          enabledHint: 'When on, ops alerts are pushed to the device configured below.',
          serverUrl: 'Server URL',
          serverUrlPlaceholder: 'https://api.day.app',
          serverUrlHint: 'Defaults to the official Bark server. If you run your own Bark server, enter its address.',
          deviceKey: 'Device Key',
          deviceKeyPlaceholder: 'Separate multiple device keys with commas, e.g. key1,key2',
          deviceKeyConfiguredPlaceholder: 'Configured. Leave blank to keep the current key.',
          deviceKeyConfigured: 'Configured',
          deviceKeyConfiguredCount: '{count} device(s) configured',
          deviceKeyHint:
            'Separate multiple device keys with commas (up to 10). Every notification is pushed to all of them. Keys are never shown again after saving; to rotate one, just enter a new list.',
          deviceKeyRequired: 'Push is enabled but no device key is configured. Enter a device key before saving.',
          titlePrefix: 'Notification name',
          titlePrefixHint: 'Shown at the start of Bark notification titles to distinguish deployments. Leave blank to use Sub2API. This is separate from the Bark notification group below.',
          group: 'Notification group',
          groupHint: 'Notifications in the same group are grouped together in Bark, keeping them apart from other pushes.',
          ...
          levelHints: {
            active: 'Normal push: lights up the screen and plays a sound.',
            timeSensitive: 'Time-sensitive: can break through Focus modes.',
            passive: 'Passive: only added to the notification list, no screen wake or sound.',
            critical: 'Critical: bypasses silent mode and Do Not Disturb. Use only for alerts that must wake someone up.',
          },
          sound: 'Sound',
          soundHint: 'Optional. A sound name supported by Bark (e.g. alarm, bell). Leave blank for the default.',
          clickUrl: 'Click-through URL',
          clickUrlHint: 'Optional. Attached to the notification; tapping it opens this address, e.g. the ops dashboard.',
          notifyOnResolve: 'Also notify when an alert resolves',
          notifyOnResolveHint: 'Send a follow-up push when the alert recovers. Turn off to push only when alerts fire.',
          lastSaved: 'Last saved: {time}',
          testConnection: 'Test connection',
          sendTest: 'Send test notification',
          testTitle: '[{name}] Bark test notification',
          testBody: 'This is a Bark test notification from “{name}”. If you received it, Bark is configured correctly.',
          serverUrlRequired: 'Enter the server URL first.',
          deviceKeyRequiredForTest: 'No device key yet, cannot send a test notification. Enter a device key first.',
          connectionOk: 'Server reachable, latency {latency} ms',
          connectionFailed: 'Connection failed',
          connectionOkNoDeviceKey: 'Server reachable (latency {latency} ms), but no device key is configured yet',
          connectionPingFailedPushOk: 'Server did not answer the ping endpoint, but push works',
          connectionFailedWithMessage: 'Connection failed: {message}',
          sent: 'Sent. Check your phone.',
          sentPartial: 'Some devices did not receive it: {success} delivered, {failed} failed',
          sendFailed: 'Send failed',
          testFailed: 'Test failed',
          resultPingOk: 'Server reachable',
          resultPingFailed: 'Server unreachable',
          resultSent: 'Test notification sent',
          resultNotSent: 'Test notification not sent',
          resultPartial: 'Some devices did not receive the test notification',
          resultDeviceSummary: '{total} device(s): {success} delivered, {failed} failed',
          deviceResultOk: 'Device {index} ({masked}): delivered',
          deviceResultFailed: 'Device {index} ({masked}): failed — {message}',
          resultLatency: 'Latency {latency} ms',
          resultStatus: 'HTTP {status}',
          saved: 'Bark push settings saved',
          saveFailed: 'Failed to save Bark push settings',
          loadFailed: 'Failed to load Bark push settings',
        },
        errorDigest: {
          title: 'Scheduled Error Digest',
          description: 'At fixed times each day, group the request errors from the previous period by API key and push a single summary through the Bark channel above.',
          enabled: 'Enable error digest',
          enabledHint: 'When on, the digest is pushed on the schedule below. It never runs while ops monitoring is disabled.',
          schedule: 'Push schedule',
          scheduleHint: 'A 5-field cron expression evaluated in the server timezone. The default 30 11,17 * * * means 11:30 and 17:30 every day.',
          topKeys: 'Keys to list',
          topKeysHint: 'List this many keys, most errors first, each with its top 3 error types. The rest are folded into "N more keys, M errors".',
          skipWhenEmpty: 'Skip when there are no errors',
          skipWhenEmptyHint: 'When on, a period without a single error is not pushed. When off, a "no errors" notification is sent anyway.',
          lastSaved: 'Last saved: {time}',
          test: 'Send test digest',
          resultPushed: 'Pushed a digest of the last 24 hours — check your phone',
          resultNotPushed: 'The digest was generated but not pushed (Bark is disabled or incomplete)',
          pushed: 'Pushed — check your phone',
          notPushed: 'Bark push is not enabled, so the digest was generated but not sent',
          testFailed: 'Test digest failed',
          saved: 'Error digest settings saved',
          saveFailed: 'Failed to save error digest settings',
          loadFailed: 'Failed to load error digest settings',
        },
        accountUsageRules: {
          title: 'Usage & Expiry Alert Rules',
          description: "Watch each account's window usage, quota, balance, today's cost and days left before expiry, plus each API key's daily usage, and push to your phone through the Bark channel above once a threshold is reached.",
          create: 'New Rule',
          createTiers: 'Create 3 Tiers',
          loading: 'Loading...',
          empty: 'No alert rules yet. Start with "New Rule" or "Create 3 Tiers".',
          loadFailed: 'Failed to load account usage alert rules',
          opsDisabled: 'Ops monitoring is currently off. These rules run on its alert engine, so enable ops monitoring (ops_monitoring_enabled) first.',
          duplicateName: 'A rule with this name already exists',
          saveSuccess: 'Alert rule saved',
          saveFailed: 'Failed to save alert rule',
          toggleFailed: 'Failed to toggle the rule',
          deleteSuccess: 'Alert rule deleted',
          deleteFailed: 'Failed to delete alert rule',
          deleteConfirmTitle: 'Delete this rule?',
          deleteConfirmMessage: 'Its alert events will be removed as well. This cannot be undone.',
          metrics: {
            windowUsedPercent: 'Window usage',
            quotaUsedPercent: 'Quota usage',
            balance: 'Account balance',
            todayCost: "Today's cost",
            expiresInDays: 'Account expiry',
            apiKeyDailyUsedPercent: 'API key daily usage',
          },
          metricHints: {
            windowUsedPercent: 'Percentage (0-100) of the 5-hour or 7-day window the account has used; recovers automatically when the window resets.',
            quotaUsedPercent: 'Percentage (0-100) of the daily / weekly / total quota used; accounts without a quota are skipped.',
            balance: 'Remaining balance of a Chinese provider account (Kimi / DeepSeek); usually paired with a "less than" threshold.',
            todayCost: 'Cost the account has accumulated today (USD).',
            expiresInDays: 'Days left before the account expires, negative once expired; usually three "less than or equal" rules at 7 / 3 / 1 days. Accounts with no expiry date are skipped.',
            apiKeyDailyUsedPercent: "Percentage (0-100) of an API key's daily limit used in the current 1-day window; keys without a daily limit are skipped. It covers every key and cannot be filtered by target.",
          },
          units: {
            daySuffix: 'd',
            daysLeft: '{value}d left',
          },
          windows: {
            h5: '5-hour window',
            d7: '7-day window',
          ...
          form: {
            createTitle: 'New Account Usage Alert Rule',
            editTitle: 'Edit Account Usage Alert Rule',
            name: 'Rule name',
            namePlaceholder: 'e.g. Codex 5h usage',
            metric: 'Metric',
            window: 'Window',
            ...
            groupAll: 'All groups',
            accounts: 'Accounts',
            accountsHint: 'Leave empty to cover every account under the platform / group above.',
            accountsPlaceholder: 'Pick an account to add',
            accountsLoading: 'Loading accounts...',
            accountsEmpty: 'No accounts to choose from',
            accountsLoadFailed: 'Failed to load accounts',
            apiKeyScopeHint: 'API key daily usage covers every key that has a daily limit; it cannot be filtered by platform / group / account.',
            operator: 'Operator',
            threshold: 'Threshold',
            severity: 'Severity',
            cooldown: 'Cooldown (minutes)',
            cooldownHint: 'After an account fires, do not push again for this many minutes.',
            enabled: 'Enable this rule',
          },
          validation: {
            nameRequired: 'Rule name is required',
            thresholdRequired: 'Threshold must be a number',
            thresholdPercentRange: 'Percentage threshold must be between 0 and 100',
          ...
          evaluate: {
            action: 'Test now',
            title: 'Test now: {name}',
            description: 'Evaluates this rule against live data without creating an alert event; tick the box to also push one Bark notification.',
            sendToBark: 'Also push to Bark',
            run: 'Run again',
            running: 'Evaluating...',
            ...
            noData: 'No usable data in scope: the accounts have no usage snapshot, no quota, or no balance record.',
            accountsTitle: 'Accounts involved',
            apiKeysTitle: 'API keys involved',
            ...
            sent: 'Pushed to Bark',
            notSent: 'Not pushed',
            sendError: 'Push failed: {error}',
            barkNotEnabled: 'Bark push is not enabled yet. Enable and save it in the "Bark Push Notifications" card above first.',
            failed: 'Test failed',
          },
          tiers: {
            title: 'Create 3 Tiers',
            description: 'Creates three rules on the same metric and scope: one push each at 40% / 60% / 80%, each recovering on its own when the window resets.',
            baseName: 'Base name',
            baseNamePlaceholder: 'e.g. Codex 5h usage',
            baseNameRequired: 'Base name is required',
            preview: 'Will create: {names}',
            run: 'Create 3 rules',
            running: 'Creating...',
            created: 'Created',
            failed: 'Failed: {error}',
            duplicate: 'Name already exists, skipped',
            summary: 'Done: {ok} created, {failed} failed',
          },
        },
      },
      features: {
        channelMonitor: {
          ...
          description: 'Passively aggregates real gateway traffic into per-platform / per-model health, error categories, and latency. When disabled, aggregation stops and the user entry is hidden.',
          configureLink: 'Configure aggregation in Channel Management > Channel Monitor',
          enabled: 'Enable Channel Monitor',
          enabledHint: 'Disabling stops minute aggregation and makes both admin and user endpoints return CHANNEL_MONITOR_DISABLED; existing config and history are kept.',
        ...
        riskControl: {
          ...
          description: 'Enable the security audit menu and the prompt audit pipeline. Disabled by default.',
          configureLink: 'Configure audit policy in Prompt Audit',
          enabled: 'Enable Security Audit',
          enabledHint: 'When off, the admin sidebar entry is hidden and prompt audit is skipped.',
