export default {
    accounts: {
      ...
      capacity: {
        ...
        load: {
          hoverHint: '悬停查看最近负载，点击打开明细',
          summary: '并发 {current}/{max} · 最近 {minutes} 分钟 {count} 次 · 最活跃密钥 {key}',
          summaryLoading: '正在读取最近负载…',
          summaryFailed: '最近负载读取失败，点击查看明细',
          noKey: '无',
          title: '容量负载明细',
          concurrency: '并发',
          waiting: '等待',
          windowMinutes: '{minutes} 分钟',
          totalRequests: '窗口内共 {count} 次请求',
          byApiKey: '按密钥',
          byModel: '按模型',
          requests: '请求明细',
          requestsLimitHint: '仅展示最近 {limit} 条',
          columns: {
            time: '时间',
            user: '用户',
            apiKey: '密钥',
          ...
          emptyRequests: '该时间窗口内没有请求记录',
          errors: '同期错误',
          errorsEmpty: '该时间窗口内没有错误记录',
          errorsFailed: '同期错误读取失败，不影响上方数据',
          delayHint: '数据来自使用记录，约有数秒延迟',
          loadFailed: '负载数据读取失败',
          retry: '重试',
          refresh: '刷新',
          count: '{count} 次'
      ...
      quotaLimitHint: '设置日/周/总使用额度（美元），任一维度达到限额后账号暂停调度。周限额默认按自然周重置（每周一 00:00，服务器时区）。Anthropic API Key 账号还可配置客户端亲和。修改限额不会重置已用额度。',
      ...
      quotaWeeklyLimitHint: '滚动窗口：从首次使用起每 7 天重置（旧方式；新设周限额默认改为固定时间的自然周）。',
      quotaTotalLimit: '总限额',
