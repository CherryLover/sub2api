        ...
        <section class="space-y-1.5" data-testid="load-drawer-requests">
          ...
          <div v-if="requestItems.length" class="overflow-x-auto rounded-lg border border-gray-200 dark:border-dark-700">
            <table class="min-w-full text-xs">
              <thead class="bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500 dark:bg-dark-700/60 dark:text-gray-400">
                <tr>
                  <th class="px-2 py-1.5 font-medium">{{ t('admin.accounts.capacity.load.columns.time') }}</th>
                  <th class="px-2 py-1.5 font-medium">{{ t('admin.accounts.capacity.load.columns.user') }}</th>
                  <th class="px-2 py-1.5 font-medium">{{ t('admin.accounts.capacity.load.columns.apiKey') }}</th>
                  <th class="px-2 py-1.5 font-medium">{{ t('admin.accounts.capacity.load.columns.model') }}</th>
                  <th class="px-2 py-1.5 text-right font-medium">{{ t('admin.accounts.capacity.load.columns.duration') }}</th>
                  <th class="px-2 py-1.5 text-right font-medium">{{ t('admin.accounts.capacity.load.columns.firstToken') }}</th>
                  <th class="px-2 py-1.5 text-right font-medium">{{ t('admin.accounts.capacity.load.columns.tokens') }}</th>
                  <th class="px-2 py-1.5 text-right font-medium">{{ t('admin.accounts.capacity.load.columns.cost') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                <tr v-for="item in requestItems" :key="item.id" class="text-gray-700 dark:text-gray-200" data-testid="load-request-row">
                  <td class="whitespace-nowrap px-2 py-1.5 font-mono text-gray-500 dark:text-gray-400">{{ formatTime(item.created_at) }}</td>
                  <td class="max-w-[160px] truncate px-2 py-1.5" :title="item.user?.email || ''">{{ item.user?.email || '-' }}</td>
                  <td class="max-w-[120px] truncate px-2 py-1.5" :title="item.api_key?.name || ''">{{ item.api_key?.name || '-' }}</td>
                  <td class="px-2 py-1.5">
                    <div class="flex items-center gap-1">
                      <span class="break-all">{{ item.model }}</span>
                      <span
                        v-if="item.stream"
                        class="rounded bg-gray-100 px-1 text-[9px] uppercase text-gray-500 dark:bg-dark-700 dark:text-gray-400"
                      >stream</span>
                    </div>
                    <div v-if="item.upstream_model && item.upstream_model !== item.model" class="break-all text-[11px] text-gray-400 dark:text-gray-500">
                      ↳ {{ item.upstream_model }}
                    </div>
                  </td>
                  <td class="whitespace-nowrap px-2 py-1.5 text-right font-mono">{{ formatMs(item.duration_ms) }}</td>
                  <td class="whitespace-nowrap px-2 py-1.5 text-right font-mono">{{ formatMs(item.first_token_ms) }}</td>
                  <td class="whitespace-nowrap px-2 py-1.5 text-right font-mono">
                    ...
                    <span v-if="item.cache_read_tokens" class="text-gray-400 dark:text-gray-500">(+{{ formatNumber(item.cache_read_tokens) }})</span>
                  </td>
                  <td class="whitespace-nowrap px-2 py-1.5 text-right font-mono">{{ formatCost(item.actual_cost) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
