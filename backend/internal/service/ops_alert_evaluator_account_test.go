...
func TestOpsAlertEvaluator_AccountTargetsFireAndResolveIndependently(t *testing.T) {
	...
	require.Equal(t, "[Sub2API] Codex 5h 用量 · P2 告警", sends[0].Msg.Title)
	...
	require.Equal(t, "[Sub2API] Codex 5h 用量 · 已恢复", sends[2].Msg.Title)
	require.Contains(t, sends[2].Msg.Body, "账号：codex-a（openai）")
	require.Contains(t, sends[2].Msg.Body, "恢复值：20%")
