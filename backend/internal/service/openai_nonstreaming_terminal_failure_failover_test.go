...
func TestNonStreamingSSEToJSON_BareErrorEventUsesConservativeClassifier(t *testing.T) {
	...
	t.Run("transient_fails_over", func(t *testing.T) {
		...
		svc := newNonStreamingFailoverService()
		// Lite 的 error 分类器只认正向瞬时文案（processing / capacity / 官方 retry 模板），
		// 不把泛化的 "please retry" 当瞬时。用 Lite 已有判定能命中的 payload 验证分派。
		data := `{"type":"error","error":{"message":"An error occurred while processing your request"}}`
