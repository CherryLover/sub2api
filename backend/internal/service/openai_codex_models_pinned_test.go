...
func TestIsPinnedCodexModelsAccountUsable(t *testing.T) {
	expired := time.Now().Add(-time.Hour)
	future := time.Now().Add(time.Hour)
	require.False(t, isPinnedCodexModelsAccountUsable(nil))
	require.False(t, isPinnedCodexModelsAccountUsable(&Account{Status: "inactive", Schedulable: true}))
	require.False(t, isPinnedCodexModelsAccountUsable(&Account{Status: StatusActive, Schedulable: false}))
	require.False(t, isPinnedCodexModelsAccountUsable(&Account{
		Status:             StatusActive,
		Schedulable:        true,
		AutoPauseOnExpired: true,
		ExpiresAt:          &expired,
	}))
	require.True(t, isPinnedCodexModelsAccountUsable(&Account{
		Status:             StatusActive,
		Schedulable:        true,
		AutoPauseOnExpired: true,
		ExpiresAt:          &future,
	}))
	require.True(t, isPinnedCodexModelsAccountUsable(&Account{Status: StatusActive, Schedulable: true}))
}
