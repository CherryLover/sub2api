...
func TestOAuthInputInternalMetadata(t *testing.T) {
	...
	tests := []struct {
		...
		{"OAuth websocket", func(b []byte) ([]byte, bool, error) {
			return normalizeOpenAIOAuthInputMetadataForAccount(b, &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth})
		}, true},
		{"API key websocket", func(b []byte) ([]byte, bool, error) {
			return normalizeOpenAIOAuthInputMetadataForAccount(b, &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey})
		}, false},
	}
...
func TestOAuthInputInternalMetadataCompatibility(t *testing.T) {
	for _, body := range []string{
		...
		t.Run(body, func(t *testing.T) {
			out, changed, err := stripOpenAIOAuthInputMetadataBody([]byte(body))
			require.NoError(t, err)
			require.False(t, changed)
			require.Equal(t, body, string(out))
			var req map[string]any
			require.NoError(t, json.Unmarshal([]byte(body), &req))
			require.False(t, stripOpenAIOAuthInputMetadataFields(req))
		})
	}
}
