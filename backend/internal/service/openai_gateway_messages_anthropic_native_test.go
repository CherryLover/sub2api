...
type nativeAnthropicUpstreamRecorder struct {
	lastBody []byte
	resp     *http.Response
}

func (u *nativeAnthropicUpstreamRecorder) Do(req *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
...
func (u *nativeAnthropicUpstreamRecorder) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func TestNativeAnthropicPassthroughRecordsOutputConfigEffort(t *testing.T) {
	...
	upstream := &nativeAnthropicUpstreamRecorder{resp: nativeAnthropicBufferedResponse()}
...
func TestNativeAnthropicPassthroughStreamRecordsEffort(t *testing.T) {
	...
	body := []byte(`{"model":"k3","max_tokens":32,"stream":true,` +
		...
		`"messages":[{"role":"user","content":"hi"}]}`)
	upstream := &nativeAnthropicUpstreamRecorder{resp: nativeAnthropicStreamResponse()}
	svc := &OpenAIGatewayService{cfg: rawChatCompletionsTestConfig(), httpUpstream: upstream}
...
func TestNativeAnthropicPassthroughNormalizesGLM53Thinking(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			upstream := &nativeAnthropicUpstreamRecorder{resp: response}
			svc := &OpenAIGatewayService{cfg: rawChatCompletionsTestConfig(), httpUpstream: upstream}
