...
// requiredCSPDirectiveValues 是无论管理员写了什么自定义策略都必须补齐的条目。
//
// 单管理员内部部署把人机验证（腾讯天御 / Turnstile / 阿里云）与支付
// （Stripe / Airwallex）整套删掉之后，它们的 script-src / style-src /
// frame-src / connect-src / worker-src 白名单只剩"无谓放宽策略"这一个作用，
// 已随功能一并移除。这里只留 Cloudflare Web Analytics：它由 Cloudflare 边缘
// 在响应里注入，与仓库代码无关，删掉会让开了 Web Analytics 的部署报 CSP 错。
...
// enhanceCSPPolicy 确保 CSP 策略包含 nonce 支持和运行时必需域名。
// 这样旧配置文件没有及时补域名时，Cloudflare Web Analytics 仍能正常加载。
