...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	_, err = integrationDB.ExecContext(ctx, "UPDATE users SET notes = 'cosmetic' WHERE id = $1", user.ID)
	require.NoError(t, err)
	require.Zero(t, count(), "non-security user update with unchanged allowed groups must not enqueue")
